...
from app import state, prediction, recommendation
...
# Let's pick an origin and destination that pass through KBS (or start there)
# to see what a recommended route array looks like.
options = recommendation.find_route_options("20587", "21642") # Rajajinagar to Papareddypalya (from user screenshot!)
if not options:
    print("Could not find route options.")
    sys.exit()
...
def test_avg_delay(severity):
    ...
    for i, stop in enumerate(route_stops):
        ...
        print(f"  Stop {i:02d} ({stop}): {d:.2f} min{is_epicenter}")
